...
2. Переходит в поиск вакансий по запросу (`--query`). Поддерживает несколько запросов через запятую — когда вакансии по одному запросу заканчиваются, переходит к следующему.
3. Пропускает вакансии, в названии которых есть нежелательные слова (`--exclude`).
4. Ищет доступные вакансии с кнопкой отклика. Пропускает вакансии с обязательными вопросами.
5. Подставляет сопроводительное письмо.
6. Не откликается на одну и ту же вакансию дважды.
7. Может работать батчем по нескольким `resumeId` параллельно.
...
```bash
npm run start:pw:headed -- --cookies ./playwright/cookies.json --resume YOUR_RESUME_ID --query "frontend,react разработчик,JS developer" --exclude "junior,стажер,php" --max 10 --maxAttempts 40 --maxFailStreak 3
```
...
5. `--maxFailStreak` число неудач подряд до перехода на следующую страницу выдачи или следующий запрос.
6. `--cover` текст сопроводительного письма.
7. `--query` текстовый запрос поиска. Поддерживает несколько через запятую: `"frontend,react разработчик,JS developer"`.
8. `--exclude` слова-исключения: вакансии с этими словами в названии пропускаются. Через запятую: `"junior,стажер,php"`.
9. `--url` прямой URL выдачи вакансий.
10. `--debug` подробные логи и debug-скриншоты.
11. `--headed` запуск с видимым окном браузера.
12. `--desktop` запуск в обычном десктопном режиме вместо мобильной эмуляции.
...
5. `--query` текстовый запрос поиска вакансий (пробрасывается в дочерние процессы).
6. `--exclude` слова-исключения (пробрасываются в дочерние процессы).
7. `--max`, `--maxAttempts`, `--maxFailStreak`, `--cookies` пробрасываются в дочерние процессы.
